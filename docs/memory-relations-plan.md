...
### フェーズ 4: 拡張と自動化
- [x] Web UI や CLI から一連の推論ワークフローを呼び出せるようツール化（`scripts/phase4Automation.ts`）
- [x] 重要メモを定期的に再評価し、新しい関係をサジェストするバッチ（スケジューラ）を紹介（`docs/inference-phase4.md` の cron 例）
- [x] 外部グラフデータベース（Memgraph 等）との同期や可視化の PoC 指針を `docs/inference-phase4.md` に記載
