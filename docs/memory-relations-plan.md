...
- [ ] グラフ探索で得た `path` 情報をチェーン形式に整形し、LLM へ「推論の根拠」として渡す
- [ ] 自動評価用に回答テンプレート（結論＋根拠＋確信度）を定義

### フェーズ 3: ルール・フィードバックの導入
- [ ] 特定タグを含む関係のみを探索するフィルタリング設定（例: 推奨サマリなら supports のみ）
- [ ] LLM の応答結果に応じて新しいリレーションを提案・保存するループを試験
- [ ] 推論失敗時に別 pivot を選び直すリトライ戦略を検証
